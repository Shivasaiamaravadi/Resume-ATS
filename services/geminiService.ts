...
export const analyzeAndReviseResume = async (
  resumeText: string,
  jobDescription: string
): Promise<ATSAnalysisResult> => {
  // ✅ Vite environment variable check (frontend-safe)
  if (!import.meta.env.VITE_API_KEY) {
    throw new Error("VITE_API_KEY environment variable not set");
  }

  // ✅ Initialize Gemini with the correct key
  ...
  const schema = {
    ...
    properties: {
      originalAtsScore: {
        type: Type.NUMBER,
        description: "ATS score for the original resume (0-100).",
      },
      revisedAtsScore: {
        type: Type.NUMBER,
        description:
          "ATS score for the revised resume (0-100), aiming for 85%+.",
      },
      feedback: {
        type: Type.STRING,
        description:
          "Concise feedback on the changes made, using markdown for lists (e.g., '* Point 1').",
      },
  ...
  try {
    ...
    // ✅ Basic validation
    if (
      typeof resultJson.originalAtsScore !== "number" ||
      typeof resultJson.revisedAtsScore !== "number" ||
      !resultJson.feedback ||
      !resultJson.revisedResume
    ) {
    ...
    throw new Error(
      "Failed to get analysis from the AI. The model may have returned an invalid response."
    );
  }
};
